...
class wizard_product_status_report(models.TransientModel):
	_name = "vit.wizard_product_status_report"
	...
	def get_report(self):
		...
		return self.env.ref('vit_marketing_status_report.product_status_report').report_action(self, data=data)

class ReportProductStatus(models.AbstractModel):
	"""Abstract Model for report template.

	for `_name` model, please use `report.` as prefix then add `module_name.report_name`.
	"""

	_name = 'report.vit_marketing_status_report.product_view'
