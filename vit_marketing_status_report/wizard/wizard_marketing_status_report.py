...
class wizard_marketing_status_report(models.TransientModel):
	_name = "vit.wizard_marketing_status_report"
	...
	def get_report(self):
		...
		return self.env.ref('vit_marketing_status_report.marketing_status_report').report_action(self, data=data)

class ReportMarketingStatus(models.AbstractModel):
	"""Abstract Model for report template.

	for `_name` model, please use `report.` as prefix then add `module_name.report_name`.
	"""

	_name = 'report.vit_marketing_status_report.project_view'
